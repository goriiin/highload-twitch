...
| :------------------------------------------------- | :-------- | :------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------- | :------------------------------------------ |
| **user**                                           | YDB       | ~1.36 ТБ            | Для разделения данных <br>на несколько серверов и<br>масштабирования записи<br><br>хэш функция для распределения<br>по шардам, популярных <br>вручную. ключ -- `user_id` | - user_id: HASH <br>- email : HASH<br>- username: HASH <br>- phone: HASH                           | master-slave<br>5 реплики<br>Полусинхронная |
| **stream**                                         | YDB       | ~20 ГБ              | -                                                                                                                                                                        | - stream_id: HASH<br>- user_id: TREE<br>- category_id: TREE <br>- status: TREE                     | master-slave<br>5 реплик<br>Полусинхронная  |
| **category**                                       | YDB       | ~8.4 КБ             | -                                                                                                                                                                        | - category_id HASH<br>- name HASH                                                                  | master-slave<br>3 реплики<br>               |
| **follow**                                         | YDB       | ~33.5 ГБ            | -                                                                                                                                                                        | -id: HASH <br>- user_id: HASH <br>- follow: HASH                                                   | master-slave<br>5 реплик<br>Полусинхронная  |
...
| **stream_<br>reaction**                            | YDB       | ~100 ГБ             | Для масштабирования записи и <br>разделения данных<br>на несколько<br>серверов и масштабирования записи<br><br>хэш функция<br>`stream_id`                                | - (stream_id, created_at): TREE                                                                    | Полусинхронная 3 реплики<br>master-slave    |
| **sessions**                                       | Tarantool | ~44.7 ГБ            | -                                                                                                                                                                        | - user_id: HASH <br>- токен: HASH                                                                  | Полусинхронная 3 реплики<br>master-slave    |
| **message**                                        | Tarantool | ~2.5 ТБ             | Для разделения данных <br>на несколько серверов<br><br>хэш функция<br>`user_id(стримера)` `stream_id`                                                                    | `(stream_id, created_at)` : TREE                                                                   | Асинхронная, 5 Реплик<br>master-slave       |
...
| **personal_stream_rec** (Tier 1 лента)             | Tarantool | ~1ТБ                | хэш функция<br>`user_id`                                                                                                                                                 | (user_id, stream_id) TREE<br>(user_id, score, stream_id) TREE                                      | Master-Slave, Асинхронная, 3 реплики        |
| **regional_category_stream_rec** (Tier 2 лента)    | Tarantool | ~2ГБ                | -                                                                                                                                                                        | (region, category_id, stream_id) TREE<br> (region, category_id, score DESC, stream_id DESC) [TREE] | Master-Slave, Асинхронная, 3 реплики        |
| **user_favorite_categories** (Tier 2 предпочтения) | Tarantool | ~50ГБ               | -                                                                                                                                                                        | (user_id, category_id): TREE<br>(user_id, preference_score DESC, category_id DESC): TREE           | Master-Slave, Асинхронная, 3 реплики        |
...
# 8. Технологии
| Технология                              | Область применения                                                                                               | Мотивационная часть (Зачем это нужно?)                                                                                                                                                                                                                                                                                                                                                                                                                                |
| :-------------------------------------- | :--------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| **S3**                                  | Хранение больших бинарных объектов                                                                               | Экономичное, масштабируемое и надежное хранение больших объемов неструктурированных данных с возможностью легкой интеграции с CDN для раздачи.                                                                                                                                                                                                                                                                                                                        |
...
| **ABR (Adaptive Bitrate Streaming)**    | Доставка видеопотоков с использованием различных кодеков и битрейтов.                                            | Обеспечение плавного воспроизведения видео у пользователей с разной скоростью интернет-соединения(если выбрано авто) и разными возможностями устройств путем нарезки видео на чанки разного качества и динамического переключения между ними на стороне клиента.<br>Для расчета пропускной способности сети пользователя, мы знаем время загрузки и размер каждого сегмента, используем алгоритм скользящего среднего. Таким образом берем следующий подходящий чанк. |
