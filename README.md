...
![image](https://github.com/user-attachments/assets/075a01e3-48cc-4bb3-aac5-e0554ed6809a)
...
| **stream**               | YDB       | ~20 ГБ              | Для масштабирования записи<br><br>хэш функция для распределения<br>по шардам <br>`user_id` `stream_id`                                                                   | - stream_id: HASH<br>- user_id: TREE<br>- category_id: TREE <br>- status: TREE                | master-slave<br>5 реплик<br>Полусинхронная  |
| **category**             | YDB       | ~8.4 КБ             | -                                                                                                                                                                        | - category_id HASH<br>- name HASH                                                             | master-slave<br>3 реплики<br>               |
| **follow**               | YDB       | ~33.5 ГБ            | Для масштабирования записи<br><br>хэш функция `user_id`                                                                                                                  | -id: HASH <br>- user_id: HASH <br>- follow: HASH                                              | master-slave<br>5 реплик<br>Полусинхронная  |
| **roles**                | YDB       | ~23 ГБ              | Для масштабирования записи<br><br>`streamer_id` (хеш)                                                                                                                    | -streamer_id: HASH                                                                            | master-slave<br>5 реплик<br>Асинхронная     |
| **category<br>\_search** | ydb       | *Переменный*        | Н/Д                                                                                                                                                                      | -category_name: HASH                                                                          | Асинхронная <br>3 реплики<br>master-slave   |
| **sessions**             | Tarantool | ~44.7 ГБ            | Для разделения данных на несколько<br>серверов<br><br>хэш функция `user_id                                                                                               | - user_id: HASH <br>- токен: HASH                                                             | Полусинхронная 3 реплики<br>master-slave    |
| **message**              | Tarantool | ~2.5 ТБ             | Для разделения данных <br>на несколько серверов<br><br>хэш функция<br>`user_id(стримера)` `stream_id`                                                                    | ПК (`message_id`), Вторичные: `(stream_id, created_at)` [для выборки], `created_at` [для TTL] | Асинхронная, 5 Реплик<br>master-slave       |
| **viewers**              | Tarantool | ~1 МБ               | Для масштабирования записи<br><br>`stream_id`                                                                                                                            | ПК (`stream_id`)                                                                              | Асинхронная, 5 Реплик<br>master-slave       |
| **users<br>\_reactions** | Tarantool | ~100 ГБ             | Для масштабирования записи и <br>разделения данных<br>на несколько<br>серверов<br><br>хэш функция<br>`stream_id`                                                         | - (stream_id, created_at): TREE<br>                                                           | Полусинхронная 3 реплики<br>master-slave    |
| **reactions**            | Tarantool | ~30 МБ              | Для масштабирования записи<br><br>хэш функция<br>`stream_id                                                                                                              | - stream_id: HASH                                                                             | Полусинхронная 5 реплик<br>master-slave     |
| **follow**               | Tarantool | ~100 МБ             | Для масштабирования записи<br><br>`user_id` (хеш)                                                                                                                        | -user_id: HASH                                                                                | Полусинхронная 5 реплик<br>master-slave     |
