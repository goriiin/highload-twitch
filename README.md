...
![image](https://github.com/user-attachments/assets/84e138ce-d938-4d9c-938a-31867b569c93)
...
Шардировать user по консистентная хэш функция для распределения по шардам, популярных вручную.

video, message, stream, reaction консистентная хэш функция для распределения по шардам
...
- username для User - тип  B‑tree
- title для stream - тип B‑tree
- name для category - тип B‑tree
- stream_id для message - тип B‑tree
