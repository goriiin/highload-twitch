...
11. [Базовый расчёт аппаратных ресурсов](#базовый-расчёт-аппаратных-ресурсов)
...
1. Резервирование
	- Инфраструктурное: 
		- Размещение инфраструктуры в нескольких географически распределенных дата-центрах ([Глобальная балансировка нагрузки](#3-глобальная-балансировка-нагрузки)). Это защищает от масштабных сбоев, затрагивающих целый регион (стихийные бедствия, крупные отключения электроэнергии).
...
| Компонент                | Пиковый RPS (оценка) | Сложность / Тип      | Базовый CPU (Ядра) | Базовый RAM (ГБ) | Базовый Диск (ТБ) |
| :----------------------- | :------------------- | :------------------- | :----------------- | :--------------- | :---------------- |
| YDB                      | 507,352              | БД (OLTP/Core)       | 100                | 50000            | 4                 |
| Tarantool                | 5,500,000            | БД (In-Memory/Cache) | 55,000             | 1,200            | 1.2               |
| Elasticsearch            | 4,000                | БД (Search)          | 40                 | 10               | 1                 |
| ClickHouse               | Н/П                  | БД (Аналитика)       | Н/П                | 1,000            | 100               |
| S3 / Minio               | Н/П                  | Хранилище (Объекты)  | -                  | -                | 502,000           |
| **Итого БД / Хранилища** |                      |                      | **~60,114**        | **~2,610**       | **~502,105**      |


### Таблица требуемых ресурсов для 1 % нагрузки

| Ресурс              | Без резерва | +15 % запас |
| ------------------- | ----------- | ----------- |
| **CPU, ядра**       | 442         | **508**     |
| **RAM, ГБ**         | 4 893       | **5 627**   |
| **Disk, ТБ**        | 7 061       | **8 120**   |
| **Network, Gbit/s** | 327         | **376**     |


###  Конфигурация серверов и число машин

#### Характеристики узла

Конфигурация на ДЦ 1%

| Конфигурация                                                                                                        | CPU (ядра) | RAM (ГБ) | Disk (ТБ) | Net (Gbit/s) |
| ------------------------------------------------------------------------------------------------------------------- | ---------- | -------- | --------- | ------------ |
| Intel Xeon Silver 4416+ (20 c/40 t) @2.0 GHz, 37.5 MB L3 ([Intel][5]) <br> + 4×16 GB DDR5, NVMe 1 TB, 10 Gbit/s NIC | **20**     | **64**   | **1**     | **10**       |

####  Расчёт числа серверов по каждому ресурсу

| Ресурс  | Потребность | Емкость узла | Машин, шт.                |
| ------- | ----------- | ------------ | ------------------------- |
| CPU     | 508 яд.     | 20 яд./узел  | 508 / 20 = 25.4 → **26**  |
| RAM     | 5 627 ГБ    | 64 ГБ/узел   | 5627 / 64 = 87.9 → **88** |
| Disk    | 8 120 ТБ    | 1 ТБ/узел    | 8120 / 1 = **8 120**      |
| Network | 376 Gbit/s  | 10 Gbit/s    | 376 / 10 = 37.6 → **38**  |


### Стоимость и амортизация

- **Цена за сервер:** 4 500 € (по аналогии с примером)
- **Общая стоимость:** 8 120 × 4 500 € = **36 540 000 €**
- **Амортизация (5 лет):** 36 540 000 € / 5 = **7 308 000 € / год**
- **Месячная амортизация:** 7 308 000 € / 12 = **609 000 € / мес**
- **Амортизация на 1 сервер:** 4 500 € / 5 / 12 = **75 € / мес**


|  DC-доля  | CPU (ядра) | RAM (ГБ) | Disk (ТБ) | Net (Gbit/s) | #серверов CPU ⌈/20⌉ |   #серверов RAM ⌈/64⌉   |   #серверов Disk ⌈/1⌉   |  #серверов Net ⌈/10⌉ |    Итог (макс)   |
| :-------: | :--------: | :------: | :-------: | :----------: | :-----------------: | :---------------------: | :---------------------: | :------------------: | :--------------: |
|  **2 %**  |    1 016   |  11 254  |   16 240  |      752     | ⌈1 016/20⌉ = **51** |  ⌈11 254/64⌉ = **176**  | ⌈16 240/1⌉ = **16 240** |   ⌈752/10⌉ = **76**  | **16 240** узлов |
| **2.5 %** |    1 270   | 14 067.5 |   20 300  |      940     | ⌈1 270/20⌉ = **64** | ⌈14 067.5/64⌉ = **220** | ⌈20 300/1⌉ = **20 300** |   ⌈940/10⌉ = **94**  | **20 300** узлов |
|  **3 %**  |    1 524   |  16 881  |   24 360  |     1 128    | ⌈1 524/20⌉ = **77** |  ⌈16 881/64⌉ = **264**  | ⌈24 360/1⌉ = **24 360** | ⌈1 128/10⌉ = **113** | **24 360** узлов |
