...
![image](https://github.com/user-attachments/assets/a46e1802-6635-43be-bd34-ca2d23847b3f)
...
| ---------------- | ----------------------------------------------------------------------------------------------------- |
| user             | Информация о пользователях (логин, контакты, аватар, пароль и прочее).                                |
| stream           | Данные о трансляциях (название, описание, теги, статус, время начала/окончания, количество зрителей). |
| message          | Сообщения пользователей в чате стрима.                                                                |
| permissions      | Права доступа, роли и описания для них.                                                               |
| role             | Связь пользователей с определёнными ролями и правами на конкретном стриме.                            |
| tag              | Метки для категоризации трансляций.                                                                   |
| cache            | Хранение пользовательских сессий                                                                      |
| S3               | внешнее хранилище файлов, таких как аватары или баннеры                                               |

### Расчет нагрузки на чтение/запись

| Таблица            | Размер данных (средний)                   | Запись (RPS)                                | Чтение (RPS)                       |
| ------------------ | ----------------------------------------- | ------------------------------------------- | ---------------------------------- |
| **user**           | ~2КБ на пользователя                      | ~1 (регистрация)                            | ~370 (просмотр профиля, логин)     |
| **stream**         | ~10КБ на запись                           | ~10K (создание, обновления статуса стрима)  | ~2.5M (просмотр стрима)            |
| **message**        | до 2КБ на сообщение                       | ~1000 (сообщений в чат)                     | ~200K (чтение чата)                |
| **permissions**    | ~1КБ на запись                            | 0                                           | ~300 (проверка разрешений)         |
| **role**           | ~0.5КБ на запись                          | ~5 (назначение ролей)                       | ~300 (проверка ролей)              |
| **tag**            | ~1КБ                                    | 0                                           | ~2000 (поиск по тегам)             |
| **cache (сессии)** | до 10КБ на пользователя                   | ~1K (авторизация, создание сессий)          | ~1M (постоянные проверки сессий)   |
| **S3 (файлы)**     | по ~200КБ на аватар/баннер/картинка тэга,<br> видео 75 ГБ   | ~100K (стриминг,загрузки аватарки/банера)                            | ~2.5M (просмотр видео)             |
