...
3. Graceful
	- Graceful Shutdown: перед отключеием контейнера, дожидаемся пока он обработает все запросы, которые уже пришли
 	- Graceful Degradation:  при дефиците ресурсов или сбоях не‑критичных сервисов переключаться на упрощённую логику, упрощаем/выключаем ML. Перестал работать чат -- выключить, упал транскодер -- сохранять напрямую в S3, при падении поиска - выкл
